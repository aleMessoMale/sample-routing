...
The only **version** supported is the v1, and, as you probably already noticed, is part of the exposed path. See **Integration** section to better understand how versioning has been managed.
...
For more info see the **Security** section.  

Unit tests and Integration tests have been created. See the **Test** section for more info.
...
**Environment property segregation** has been obtained through the use of Spring profiles. See the **Environment segregation** section for more details.
...
At the time of writing, only v1 is supported. If version is not supported, Message is redirected to a particular channel and managed from a service activator for a specific management. Main entry point for the supported version for the country-currency service is the currencyFacade. See **Multi-layer project** section for more detail regarding the project structure.  
...
For further details about functional requirements, see the section **Test**, in a particular way the integration one. 
...
Unit test are relative to the service-layer (see **Multi-layer project** Section for multi-layer explaination) and Click [here](./src/test/java/test/com/amazingsoftware/integration/sample/rest/ServiceLayerTest.java) to see it in detail.
